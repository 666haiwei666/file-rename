...
const teamplePath = 'https://normal-test-use.oss-cn-beijing.aliyuncs.com/file-rename-cli/teample.yaml'
    ...
    return new Promise((reslove, reject) => {
        request(teamplePath, function (error, response, body) {
            if (response && response.statusCode == 200) {
                ...
                spinner.error('Download failed, please check your network')
                reject()
            }
        });
    })
...
async function writeFile(content, path = configPath) {
    let error = await fse.writeFileSync(path, content)
    if (error) {
        throw error
    } else {
...
async function readConfigPath() {
    try {
        let stats = await getFileStat(configPath)
        if (!stats.isFile()) return false
    } catch {
        return false
    }
    let string = await fse.readFileSync(configPath, { encoding: 'utf-8' })
    if (string) {
